...
We see that the logistic regression model scores the lowest CV-error of our three models. Thus we choose the logistic model to perform model assessment on the test data. This emulates real-world efficacy as all test observations were left out of the model-selection process. 

### Model assessment

We now perform model assessment by classifying the observations from the test set using the logistic regression model and comparing the results to the true classifications. 

```{r}
pred.probs <- predict(longitudinal.logistic, newdata = longitudinal.test)
predictions <- pred.probs > .5 # use 0.5 cutoff for logistic
