...
The results from the test data show a good predictive accuracy with a misclassification rate of `r misc*100`%. There were `r confusion[1,2] ` false negatives and `r confusion[2,1]` false positive, giving a sensitivity of `r sens*100`% and specificity of `r spec*100`%. This means we're confident about our positive diagnoses and less confident about our negative diagnoses. For the purpose of predicting AD, being uncertain about a negative diagnosis is problematic, as this makes the model unsuitable for use as a pre-screening test. On the other hand, the high specificity counts in favor of the models usefulness.
