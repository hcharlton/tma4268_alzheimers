...
for (m in 1:p){
  cv_err_m <- matrix(nrow = 10, ncol = B)
  for (i in 1:10) {
    model <- randomForest(Demented ~., data = longitudinal.train,
                        mtry = m, ntree = B)
    cv_err_m[i] <- model$err.rate[B,1]
  }
  oob.df$oob.err[m] <- colMeans(cv_err_m) # save oob error
}
...
Here we see that the best value for $m$ is $m = $ `r oob.df$m[which.min(oob.df$oob.err)]`. Thus our best fit of a random forest uses $B = 5000$ trees and $m = $ `r oob.df$m[which.min(oob.df$oob.err)]` randomly chosen predictors. 
