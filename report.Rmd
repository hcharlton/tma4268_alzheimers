...
accuracy = c()
for (i in 1:length(C)) {
  long.svm <- svm(Demented~.,data=longitudinal.train,kernel="linear",cost=C[i],cross=10)
  accuracy <- c(accuracy,long.svm[29])
}
plot(C,accuracy)
long.svm <- svm(Demented~.,data=longitudinal.train,kernel="linear",cost=14)
long.predict.svm <- predict(long.svm, newdata=longitudinal.test,type="class")
table(long.predict.svm,longitudinal.test$Demented)


C = c(1:30)
accuracy = c()
for (i in 1:length(C)) {
  cross.svm <- svm(Demented~.,data=cs.train,kernel="linear",cost=C[i],cross=10)
  accuracy <- c(accuracy,cross.svm[29])
}
plot(C,accuracy)
Cost <- which.max(accuracy)
print(Cost)
cross.svm <- svm(Demented~.,data=cs.train,kernel="linear",cost=Cost)
cross.svm.predict <- predict(cross.svm,newdata=cs.test,type="class")
table(cross.svm.predict,cs.test$Demented)
