...
backwards.gbm <- function(trainx, trainy, B, d, shrinkage) {
  ...
  # select best model from each of the p models from AIC, but we can't use AIC since tree-based models are non-parametric -> we use cross validation to find the best one
  # for (k in ncol(trainx):2) {
  #   LL <- sum(log(Predicted probabilities for values entered as 1)) + sum(log(1 - Predicted probabilities for values entered as 0))
  #   AIC <- - 2*LL 
  # }
  # LL <- sum(log(Predicted probabilities for values entered as 1)) + sum(log(1 - Predicted probabilities for values entered as 0))
...
best_subset1 <- backwards.gbm(trainx, trainy, 1000, 2, 0.1)
best_subset2 <- backwards.gbm(trainx, trainy, 5000, 1, 0.1)
