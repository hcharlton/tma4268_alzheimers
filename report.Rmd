...
cs.prune <- prune.misclass(cs.tree , best = 2)
plot(cs.prune)
...
```{r}
# Bagging, cross-sectional
set.seed(1337)
cs.bag <- randomForest(Demented ~., cs.train, mtry = 5, importance = TRUE)
summary(cs.bag)
plot(cs.bag)
yhat.bag <- predict(cs.bag, newdata = cs.test, type = "class")
table(pred = yhat.bag, truth = cs.test$Demented) # 0.8648649
...
```{r}
# Random forest, cross-sectional
set.seed(1337)
cs.rf <- randomForest(Demented ~., cs.train, importance = TRUE)
summary(cs.rf)
plot(cs.rf)
yhat.rf <- predict(cs.rf, newdata = cs.test, type = "class")
table(pred = yhat.rf, truth = cs.test$Demented) # 0.8108108
...
set.seed(1337)
cs.train.boost <- mutate(cs.train, Demented = ifelse(Demented == "FALSE", 0,1))
cs.boost <- gbm(Demented ~., data = cs.train.boost, 
                          distribution = "bernoulli", n.trees = 5000, interaction.depth = 2)
summary(cs.boost)

cs.test.boost <- mutate(cs.test, Demented = ifelse(Demented == "FALSE", 0,1))
yhat.boost <- predict(cs.boost, newdata = cs.test.boost, n.trees = 5000, type = "response")
yhat.boost <- ifelse(yhat.boost > 0.5, 1,0)
table(pred = yhat.boost, truth = cs.test.boost$Demented) # 0.8648649
```
