...
data can tend to become more linearly seperable. So a linear kernel is a natural choice.
...
# Linear kernel
C = c(1e-4,1e-3,1e-2,1e-1,1e-0,1e+1,1e+2,1e+3,1e+4)
best_cost_lin = c()
for (j in 1:10) {
  accuracy = c()
  for (i in 1:length(C)) {
    long.svm <- svm(Demented~.,data=st.long.train,kernel="linear",cost=C[i],cross=10)
...
for (j in 1:10) {
  accuracy = c()
  for (i in 1:length(C)) {
    long.svm <- svm(Demented~.,data=st.long.train,kernel="radial",cost=C[i],cross=10)
    accuracy <- c(accuracy,long.svm[29])
  }
  index <- which.max(accuracy)
  best_cost_rad <- c(best_cost_rad,C[index])
}
print("Radial C:")
print(best_cost_rad)
```
Runtime is horrible for the linear kernel. Both models prefer costs >= 1.
A cost of 1 seems to be most prevalent in both models. 

A SVM with radial kernel also has a tuning parameter gamma
```{r}
