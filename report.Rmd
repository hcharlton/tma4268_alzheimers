...
## CV for selecting best model with nested feature selection
...
AIC.df <- data.frame(variables = 1:13)
for(i in 1:10){
    #get test index
    ...
    #train model, 
    train.best.aic <- bestglm(train.data, family = binomial, IC = "AIC", method ="forward")
    # train.best <- train.best.aic$BestModel
    #organize useful data of model
    # train.aic.subsets <- 
    #   train.best.aic$Subsets %>% 
    #   data.frame() %>% 
    #   cbind(variables = 1:nrow(train.best.aic$Subsets))
    AIC.df <- cbind(AIC.df, i <- train.best.aic$Subsets$AIC)
    #make df of iterated AIC 
    }

AIC.df.long <- melt(AIC.df, id.vars = "variables")

ggplot(AIC.df.long, aes(x = value, y = id)) + 
    geom_point() +
    facet_wrap(~ variable)


longitudinal.best.aic <- bestglm(longitudinal.train, family = binomial, IC = "AIC", method = "exhaustive")
logistic.best <- longitudinal.best.aic$BestModel
...
## CV for evaluating the performance of fitting the logistic model, using the AIC subset selection on the entire training set
...
#make n folds
n <- 5
folds <- cut(seq(1,nrow(longitudinal.train.rand)),breaks=5,labels=FALSE)
...
logistic.best.confusion.cumulative <- matrix(data = c(0,0,0,0), nrow=2,ncol=2)
...
for(i in 1:n){
    ...
    # train.best.aic <- bestglm(train.data, family = binomial, IC = "AIC", method ="forward")
    # train.logistic.best <- longitudinal.best.aic$BestModel
    train.logistic.best <- glm(Demented ~ Age + MMSE + nWBV + dMMSE, data = train.data, family = binomial)
    #test model on test partition
    logistic.best.probs <- predict(train.logistic.best, newdata = test.data, type = "response")
    #get prediction based on logistic probability output
    logistic.best.pred <- ifelse(logistic.best.probs > 0.5, 1, 0)
    # generate confusion table
    logistic.best.confusion <- table(logistic.best.pred, test.data$Demented)
    rownames(logistic.best.confusion) <- c('pred 0','pred 1')
    colnames(logistic.best.confusion) <- c('true 0','true 1')
    # generate sensitivity and specificity
    logistic.sens <- logistic.best.confusion[2,2] / sum(logistic.best.confusion[,2])
    logistic.spec <- logistic.best.confusion[1,1] / sum(logistic.best.confusion[,1])
    #append to persistent vectors
    sens.vec <- c(sens.vec,logistic.sens)
    spec.vec <- c(spec.vec,logistic.spec)
    logistic.best.confusion.cumulative <- logistic.best.confusion + logistic.best.confusion.cumulative
}
sens.cumulative <- logistic.best.confusion.cumulative[2,2] / sum(logistic.best.confusion.cumulative[,2])
spec.cumulative <- logistic.best.confusion.cumulative[1,1] / sum(logistic.best.confusion.cumulative[,1])
accuracy <- c(sens.cumulative, spec.cumulative)
```
